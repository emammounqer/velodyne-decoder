...
def packets_decoder(b_packets: Iterable[bytes]) -> Generator[tuple[Frame | None, Any], None, None]:
            ...
            if last_azimuth < previous_azimuth:
                ...
                for i in range(len(packet_data.data_blocks)):
                    ...
                    if curr_azimuth < previous_azimuth: 
                        curr_packet_data : PacketData = PacketData(packet_data.data_blocks[:i], packet_data.time_stamp, packet_data.return_mode, packet_data.factory_model)
                        frame_packets.append(curr_packet_data)
                        yield (Frame(curr_frame_num, frame_packets), None)
                        curr_frame_num += 1

                        next_packet_data : PacketData = PacketData(packet_data.data_blocks[i:], packet_data.time_stamp, packet_data.return_mode, packet_data.factory_model)
